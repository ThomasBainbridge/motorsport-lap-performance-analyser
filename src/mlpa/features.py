...
def _robust_entry_speed(speed: np.ndarray, brake: np.ndarray, distance: np.ndarray) -> float:
    """Estimate corner entry speed more robustly than a single boundary sample.

    Preference order:
    1. Mean speed over a short window ending at brake onset.
    2. Mean speed over the first 15% of the segment.
    3. First available speed sample.
    """
...
def _extract_interval_features(
    ...
    *,
    ...
    throttle = segment.get(f"{prefix}_Throttle", pd.Series(np.full(len(segment), 100.0))).to_numpy(dtype=float)
    brake = segment.get(f"{prefix}_Brake", pd.Series(np.zeros(len(segment)))).to_numpy(dtype=float)
    ...
    entry_speed = _robust_entry_speed(speed, brake, distance)
    ...
    return {
        ...
        f"{prefix}_min_speed_kph": float(np.min(speed)),
        f"{prefix}_exit_speed_kph": float(speed[-1]),
...
def compute_segment_features(
    ...
    *,
    ...
    for row in segments_df.itertuples(index=False):
        ...
        combined["brake_start_delta_m"] = (
            combined["cmp_brake_start_distance_m"] - combined["ref_brake_start_distance_m"]
        )
        ...
        combined["mean_throttle_delta_pct"] = (
            combined["cmp_mean_throttle_pct"] - combined["ref_mean_throttle_pct"]
        )
